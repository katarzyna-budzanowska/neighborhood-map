...
class LocationInformation extends Component {
  ...
  getImages = () => {
    ...
    fetch(`https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${KEY}&tags=${tags}&tag_mode=all&per_page=${num}&page=1&format=json&nojsoncallback=1`).then(res => res.json()).then((result) => {
      ...
      this.setState({isLoaded: true, pictures});
    }, (error) => {
  ...
  */
  ...
  render() {
    ...
    if (error) {
      ...
      return (<div className="App-location-information" aria-label="Locations pictures">
        <GridList cellHeight={180} cols={1} classes={{
            root: "App-location-information-list"
          }}>
          {
            pictures.map(p => (<GridListTile classes={{
